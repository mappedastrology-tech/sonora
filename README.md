...
are generated by `npm run assets`: an ink arch on a plain paper tile.

The 16, 32 and 48px versions are **drawn** at each size rather than resized from
the brand artwork, and the 180px home-screen icon keeps the full mark with its
two crescents. That split is the fix for the second thing that went wrong here.
The brand mark is portrait — 0.69 wide for its height — so fitting it into a
square fits it by height: at 16px the arch came out 7 pixels wide, running edge
to edge top and bottom, with the crescents flattened into grey smudges. It
measured perfectly (paper in the corners, ink in the middle) and looked like a
bruise. `npm run verify` now checks the geometry instead: a mark that touches the
tile edge, fills more than 90% of its height, or is too small to identify fails
the build.
